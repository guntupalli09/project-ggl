-- Email System Setup - SAFE VERSION (Handles existing objects)
-- This script safely creates the email system with proper error handling

-- ========================================
-- STEP 1: Add missing columns to user_settings if needed
-- ========================================
...
-- ========================================
-- STEP 2: Create Email Workflows Table (Safe)
-- ========================================
...
-- ========================================
-- STEP 3: Create Email Logs Table (Safe)
-- ========================================
...
-- ========================================
-- STEP 4: Create Email Templates Table (Safe)
-- ========================================
...
-- ========================================
-- STEP 5: Create Email Campaigns Table (Safe)
-- ========================================
...
-- ========================================
-- STEP 7: Create Updated_at Trigger Function (Safe)
-- ========================================
...
-- ========================================
-- STEP 8: Create Triggers (Safe)
-- ========================================

-- Drop existing triggers if they exist, then create new ones
DROP TRIGGER IF EXISTS update_email_workflow_settings_updated_at ON email_workflow_settings;
...
-- ========================================
-- STEP 9: Enable Row Level Security (Safe)
-- ========================================
...
-- ========================================
-- STEP 10: Create RLS Policies (Safe - Drop and Recreate)
-- ========================================

-- Email Workflow Settings Policies
DROP POLICY IF EXISTS "Users can view their own email workflow settings" ON email_workflow_settings;
CREATE POLICY "Users can view their own email workflow settings" ON email_workflow_settings
    FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own email workflow settings" ON email_workflow_settings;
CREATE POLICY "Users can insert their own email workflow settings" ON email_workflow_settings
    FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own email workflow settings" ON email_workflow_settings;
CREATE POLICY "Users can update their own email workflow settings" ON email_workflow_settings
    FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own email workflow settings" ON email_workflow_settings;
...
-- ========================================
-- STEP 11: Insert Default Templates (Safe - Only if not exists)
-- ========================================

-- Only insert default templates if they don't already exist
INSERT INTO email_templates (name, niche, business_type, campaign_type, subject_template, content_template, is_default, variables) 
SELECT * FROM (VALUES
-- Salon Templates
...
) AS v(name, niche, business_type, campaign_type, subject_template, content_template, is_default, variables)
WHERE NOT EXISTS (
    SELECT 1 FROM email_templates WHERE name = v.name
);
...
BEGIN
    ...
    RAISE NOTICE 'Tables: email_workflow_settings, email_logs, email_templates, email_campaigns';
    RAISE NOTICE 'Added niche and business_type columns for better organization';
    RAISE NOTICE 'Indexes, triggers, and RLS policies have been applied';
    RAISE NOTICE 'Default email templates have been inserted (if not already present)';
