...
export async function markLeadResponded(leadId: string, responseNotes?: string) {
  try {
    ...
    const currentState: RAFGSState = (lead?.rafgs_state as RAFGSState) ?? RAFGSState.NEW

    // Transition FSM state on inbound response
    let nextState: RAFGSState
    try {
      nextState = transitionState(currentState, RAFGSEvent.INBOUND_RECEIVED)
    } catch (err) {
      // Invalid transition (e.g., already ENGAGED or CLOSED) - keep current state
